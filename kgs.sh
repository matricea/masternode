...
if [[ $AGREE =~ "y" ]] ; then
echo -e ${GREEN}"Please Enter Your Masternodes Private Key for the node:"${NC}
read privkey
...
cd /root
sudo mkdir /root/.kgs
sudo touch /root/.kgs/kgs.conf
echo -e "${GREEN}Configuring Wallet for first node${NC}"
echo "rpcuser=user"`shuf -i 100000-10000000 -n 1` >> /root/.kgs/kgs.conf
echo "rpcpassword=pass"`shuf -i 100000-10000000 -n 1` >> /root/.kgs/kgs.conf
echo "rpcallowip=127.0.0.1" >> /root/.kgs/kgs.conf
echo "server=1" >> /root/.kgs/kgs.conf
echo "daemon=1" >> /root/.kgs/kgs.conf
echo "maxconnections=250" >> /root/.kgs/kgs.conf
echo "masternode=1" >> /root/.kgs/kgs.conf
echo "rpcport=17711" >> /root/.kgs/kgs.conf
echo "listen=0" >> /root/.kgs/kgs.conf
echo "externalip=$(hostname  -I | cut -f1 -d' '):17700" >> /root/.kgs/kgs.conf
echo "masternodeprivkey=$privkey" >> /root/.kgs/kgs.conf
sleep 5
fi
echo "Syncing the node, please wait...";
kgsd -daemon
sleep 10
until kgs-cli mnsync status | grep -m 1 '"IsBlockchainSynced": true,'; do sleep 1 ; done > /dev/null 2>&1
echo -e ${GREEN}"The masternode is fully synced. Your masternode is running!"${NC}
echo ""
echo -e ${GREEN}"Congrats! Your KingsCoin Masternode are now installed and started. Please wait from 10-20 minutes in order to give the masternode enough time to sync, then start the node from your wallet, Debug console option"${NC}
